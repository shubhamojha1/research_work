...
class Ant:
    ...
    def select_feature(self, feature_pheromones):
        probabilities = feature_pheromones * ~self.visited_features + 0.000001
...
df = pd.read_csv("../data/borg_traces_data_preprocessed_100.csv")
df = df.fillna(0)
...
print(best_features)
############################################################
# 1. Initialize pheromone trails one each path
# 2. For each ant, constrcut a solution, (ie select a subset of features) using a probabilistic
#     rule that favors paths with higher pheromone levels
# 3. After all ants have constructed a solution, update the pheromone trails to 
#     reinforce paths that are part of good solutions
